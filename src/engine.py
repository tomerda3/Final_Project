...
class Engine:
    ...
    def test_model(self, request_from_server=False) -> List[str]:
        ...
        if self.is_regression:
            ...
            return continuous_predictions[0]
...
def construct_hhd_engine(base_dir, image_shape, request_from_server=False, is_transformer=None) -> Engine:
    ...
    if not request_from_server:
        ...
        engine.load_images(data_type='train', image_filename_col='File', label_col='Age', clean_method=HHD)
        engine.load_images(data_type='test', image_filename_col='File', label_col='Age', clean_method=HHD)
